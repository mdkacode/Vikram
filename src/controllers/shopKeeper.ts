 ...
 * @param res
 */
export const addShopKeeper = async (
  req: Request = null,
  res: Response = null
) => {
  ...
  await newStore.save((err, storeDoc) => {
    if (err) {
      ...
      res
        .status(500)
        .jsonp({ message: "Store Creation Failed !!", error: err });
    } else {
      ...
      shopKeeper.save((err, doc) => {
        if (err) {
          ...
          infoLog("addShopKeeper => RESPONSE SUCCESS", [
            req.body,
            req.query,
            doc,
          ]);
...
export const validateShopKeeper = async (
  req: Request = null,
  res: Response = null,
  next: NextFunction
) => {
  const validData = await ShopKeeper.find().where({
...
export const deleteShopKeeper = async (
  ...
  try {
    ...
    if (doc.deletedCount > 0) {
      ...
      infoLog("deleteShopKeeper => NO RECORD FOUND", [
        req.body,
        req.query,
        doc,
      ]);
  ...
  ShopKeeper.findOneAndUpdate(
    ...
    (err: object) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ message: [], item: "Something went Wrong" });
      }
    }
  ).then((doc: object) => {
    if (!doc) {
      ...
      return res
        .status(204)
        .json({ message: [], item: "Requested Element Not Found !!" });
    } else {
      infoLog("updateShopKeeper", [req.body, req.query, doc]);
      return res
        .status(200)
  ...
  ShopKeeper.find()
    ...
    .exec((err, doc) => {
      if (err) {
        ...
        return res
          .status(500)
          .jsonp({ messge: [], error: "Something Went Wrong !!" });
      } else {
        {
            ...
            if (doc[t]._id) {
              ...
              if (fs.existsSync(IMAGE_URI + doc[t]._id)) {
                fs.readdirSync(IMAGE_URI + doc[t]._id).forEach((file) => {
                  imageSource.push(
                    `${SERVER_IP}static/${doc[t]._id + "/" + file}`
                  );
                });
              } else {
