 ...
 * @param res
 */
export const addUserAddedCart = async (
  req: Request = null,
  res: Response = null
) => {
  ...
  userAddedCart.save((err, doc) => {
    if (err) {
      ...
      infoLog("addUserAddedCart => RESPONSE SUCCESS", [
        req.body,
        req.query,
        doc,
      ]);
...
export const findUserAddedCart = async (
  req: Request = null,
  res: Response = null
) => {
          ...
          if (doc["carts"]) {
            ...
            doc.carts.prodcuts.map((product) => {
              ...
              if (product.method !== "") {
                ...
                productsArray.push({
                  shopName: product.storeId,
                  orderDate: product.orderDate || "",
                  products: product.prodcucts,
                  total: amount < 500 ? amount + 10 : amount,
                });
...
export const updateUserAddedCart = async (
  req: Request = null,
  res: Response = null
) => {
