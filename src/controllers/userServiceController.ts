...
export const adduserService = async (
  ...
  infoLog("LOGGINOTP", [uniqueNumber]);
      ...
      if (!user) {
        ...
        newUser.save((err, user) => {
          if (err) {
            ...
            const cart = new UserAddedCart({
              storeId: req.body.phone + "cart",
        ...
        User.updateOne(
          ...
          function (err: any, affected: any, user: any) {
            if (err) {
              ...
              res
                .status(500)
                .json({ error: "something went wrong", trace: err });
            } else {
...
export const validateuserService = async (
  ...
  const validData = await User.find().where({
    phone: req.body.phone,
    OTP: req.body.otp,
  });
...
export const deleteuserService = async (
  ...
  try {
    ...
    if (doc.deletedCount > 0) {
      ...
      infoLog("deleteuserService => NO RECORD FOUND", [
        req.body,
        req.query,
        doc,
      ]);
